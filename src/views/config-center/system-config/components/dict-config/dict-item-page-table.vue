<template>
  <div class="h-full">
    ...
    >
      <template #actions>
        <NButton v-if="hasAuth('sys:dict:item:add')" type="primary" @click="handleAdd">
          {{ $t('common.add') }}
        </NButton>
        <NButton v-if="hasAuth('sys:dict:item:delete')" type="error" :disabled="!checkedRowKeys.length" @click="handleBatchDelete">
          {{ $t('common.delete') }}
        </NButton>
      </template>
    </CommonTable>
