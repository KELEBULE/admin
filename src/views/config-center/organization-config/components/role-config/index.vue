<template>
  <div class="rounded-10px p-10px h-full bg-container">
    ...
    >
      <template #actions>
        <NButton v-if="hasAuth('sys:role:add')" type="primary" @click="handleAdd">新增</NButton>
        <NButton v-if="hasAuth('sys:role:delete')" type="error" :disabled="!checkedRowKeys.length" @click="handleBatchDelete">删除</NButton>
