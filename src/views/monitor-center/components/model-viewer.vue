    ...
    gltf => {
      ...
      currentModel.traverse(child => {
        if (child instanceof THREE.Mesh) {
          ...
          const meshName = child.name || `part_${meshList.length + 1}`;
...
function highlightPartByNodeName(modelNodeName: string) {
  const mesh = meshList.find(m => m.name === modelNodeName || m.userData.name === modelNodeName);
  if (mesh) {
    highlightPart(mesh);
    focusOnMesh(mesh);
  }
...
function focusOnMesh(mesh: THREE.Mesh) {
  ...
  animateCameraTo(targetPosition, center, 1200);
}

function animateCameraTo(targetPosition: { x: number; y: number; z: number }, lookAt?: { x: number; y: number; z: number }, duration: number = 1200) {
